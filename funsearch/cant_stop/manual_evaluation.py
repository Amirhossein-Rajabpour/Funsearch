...
def get_custom_action(game):
  ...
  def column_benefit(col):
    cell_values = [game.board_game.board[col][cell].markers for cell in range(len(game.board_game.board[col]))]
    return sum([sum(cell) for cell in cell_values])
  
  def heuristic(move):
    total_value = 0
    if isinstance(move, tuple):
      for col in move:
        total_value += column_benefit(col)
    return total_value
  
  sorted_moves = sorted(available_actions, key=heuristic, reverse=True)
  ...
  for move in sorted_moves:
    if isinstance(move, tuple):
      total_value = heuristic(move)
      
      # Use a more sophisticated heuristic to determine the "best" move.
      # For example, prefer moves that lead to higher column positions or avoid lower columns.
      if total_value > max_value:
        max_value = total_value
        best_move = move
  
  return best_move if best_move is not None else random.choice(available_actions)
...
def get_action(game: Game) -> Union[Tuple[int], str]:
  ...
  def heuristic(move):
    ...
    if isinstance(move, tuple):
      for col in move:
        cell_values = [game.board_game.board[col][cell].markers for cell in range(len(game.board_game.board[col]))]
        total_value += sum([sum(cell) for cell in cell_values])
    return total_value
...
for i in range(3,4):
  print(f"Winrate against player {i}: {evaluate(i)}")
