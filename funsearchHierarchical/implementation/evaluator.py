...
class Evaluator:
  ...
  def __init__(
    ...
    self._function_to_evolve2 = function_to_evolve2
  ...
  def analyse(
    ...
    if(funcbool == False):
      ...
      program_code = self._database.get_program_sample()
      ...
      new_function, program = _sample_to_program(
          sample, version_generated, self._template, self._function_to_evolve2)
      ...
      for current_input in self._inputs:
        test_output, runs_ok = self._sandbox.run(
            program, self._function_to_run, current_input, self._timeout_seconds)
        if (runs_ok and not _calls_ancestor(program, self._function_to_evolve2)
            and test_output is not None):
          if not isinstance(test_output, (int, float)):
            raise ValueError('@function.run did not return an int/float score.')
          scores_per_test[current_input] = test_output
        LOG(f"Runs ok: {runs_ok}")
        LOG(f"Test output: {test_output}")
        LOG(f"Call ancestor: {_calls_ancestor(program, self._function_to_evolve2)}")
        LOG(f"Scores per test: {scores_per_test}")
      if scores_per_test:
        LOG(f"Registering program: {new_function}\n")
        self._database.register_program(new_function, island_id, scores_per_test)
