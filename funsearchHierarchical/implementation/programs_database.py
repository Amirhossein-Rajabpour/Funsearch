...
class ProgramsDatabase:
  ...
  def __init__(
    ...
    self._function_to_evolve2: str = function_to_evolve2

    # Initialize empty islands.
    self._islands: list[Island] = []
    for _ in range(config.num_islands):
      self._islands.append(
          Island(template, function_to_evolve, function_to_evolve2, config.functions_per_prompt,
  ...
  def get_prompt(self, function_switch) -> Prompt:
    """Returns a prompt containing implementations from one chosen island."""
    island_id = np.random.randint(len(self._islands))
    code, version_generated = self._islands[island_id].get_prompt(function_switch)
...
class Island:
  ...
  def get_prompt(self, function_switch) -> tuple[str, int]:
    ...
    sorted_implementations = [implementations[i] for i in indices]
    version_generated = len(sorted_implementations) + 1
    return self._generate_prompt(sorted_implementations, function_switch), version_generated

  def _generate_prompt(
      self,
      implementations: Sequence[code_manipulation.Function], function_switch) -> str:
    ...
    if(function_switch == False):
      ...
      for i, implementation in enumerate(implementations):
        new_function_name = f'{self._function_to_evolve2}_v{i}'
        implementation.name = new_function_name
        # Update the docstring for all subsequent functions after `_v0`.
        if i >= 1:
          implementation.docstring = (
             f'Improved version of `{self._function_to_evolve2}_v{i - 1}`.')
        # If the function is recursive, replace calls to itself with its new name.
        implementation = code_manipulation.rename_function_calls(
            str(implementation), self._function_to_evolve2, new_function_name)
        versioned_functions.append(
            code_manipulation.text_to_function(implementation))

      # Create the header of the function to be generated by the LLM.
      next_version = len(implementations)
      new_function_name = f'{self._function_to_evolve2}_v{next_version}'
      header = dataclasses.replace(
          implementations[-1],
          name=new_function_name,
          body='',
          docstring=('Improved version of '
                    f'`{self._function_to_evolve2}_v{next_version - 1}`.'),
      )
      versioned_functions.append(header)
    ...
    if self._prompt_template is not None:
      # Filter out functions with names matching the new versioned functions
      existing_functions = [
          func for func in self._prompt_template.functions
          if not any(func.name in vf.name for vf in versioned_functions)
      ]
      # Combine the filtered existing functions with the new versioned functions
      updated_functions = existing_functions + versioned_functions
      prompt = dataclasses.replace(self._prompt_template, functions=updated_functions)
      return str(prompt)
