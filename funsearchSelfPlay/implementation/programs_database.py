...
class ProgramsDatabase:
  ...
  def __init__(
    ...
    self._evaluation_program: code_manipulation.Function | None = None
  ...
  def get_program_sample(self, island_id = None) -> Prompt:
    """Returns the best sample program previously generated."""
    ...
    # Ignore the first two lines of the best program (the function header and docstring).
    function = str(self._best_program_per_island[island_id]).split('\n')[2:]
    # Join the lines back together.
    return '\n'.join(function)

  def get_evaluation_program(self, island_id = None) -> Prompt:
    if island_id is None:
      island_id = np.random.randint(len(self._islands))
    if self._evaluation_program is None:
      return "return random.choice(game.available_moves())"
    function = str(self._evaluation_program).split('\n')[2:]
    return '\n'.join(function)
  ...
  def _register_program_in_island(
    ...
    # Update the evaluation program if the score was greater than the threshold.
    if score > self._config.score_improvement_threshold:
      self._evaluation_program = program
      logging.info('Evaluation program of island %d updated', island_id)
      # Also reset all the islands to only have the evaluation program.
      for i in range(len(self._islands)):
        ...
        # Use scores per test, but set all scores to 0.5 (equal chance of winning/losing).
        scores_per_test = {k: 0.5 for k in scores_per_test.keys()}
        self._register_program_in_island(program, i, scores_per_test)
